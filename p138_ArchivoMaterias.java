...
public class p138_ArchivoMaterias {
    public static ArrayList<String> leeDatos() {
        ...
        System.out.println("Introduce la lista de materias cursadas");
        while (true) {
            dato = obj.nextLine();
            if (!dato.isEmpty()) {
                datos.add(dato);
    ...
    public static void main(String[] args) {
        ...
        System.out.flush();
        ArrayList<String> materias = leeDatos(), Materias;
        try {
            grabarArchivo(materias, "materias.txt");
            System.out.println("Leyendo los datos del archivo ...");
            materias = leerArchivo("materias.txt");
            materias.forEach(n -> System.out.println(n));
        } catch (Exception e) {
